...
export const inscreverCurso = async (req, res) => {
    ...
    try {
        ...
        if (curso.senha_acesso && curso.senha_acesso !== senha_acesso) {
...
export const criarAula = async (req, res) => {
    ...
    try {
        ...
        // Adicionar lista apenas se lista_id existir e não for vazio
        if (lista_id && lista_id !== '' && lista_id !== 'null') {
            dataParaCriar.lista = { connect: { id: parseInt(lista_id) } };
        }
        
        const aula = await prisma.aula.create({ data: dataParaCriar });
        ...
        if (inscricoes.length > 0) {
            const notificacoes = inscricoes.map(insc => ({
                tipo: 'nova_aula',
                titulo: 'Nova aula disponível',
                mensagem: `Uma nova aula "${titulo}" foi adicionada ao curso.`,
        ...
        // Retornar resposta JSON (importante para o frontend não duplicar)
...
export const criarAtividade = async (req, res) => {
    ...
    try {
        ...
        const atividade = await prisma.atividade.create({ data: dataParaCriar });
...
export const criarMaterial = async (req, res) => {
    ...
    try {
        ...
        const material = await prisma.material.create({ data: dataParaCriar });
