...
// Rota: GET /admin/usuarios (CORRIGIDO: Removido 'mode: insensitive')
export const listarUsuarios = async (req, res) => {
  try {
    ...
    if (searchTerm) {
      whereCondition.nome = {
        contains: searchTerm, 
        // mode: 'insensitive', // <-- LINHA REMOVIDA
      };
    }
    ...
    console.error("Erro ao listar usuários:", error); // Mantém o log do erro original
    // Renderiza a página com uma mensagem de erro genérica, mas mantém os filtros/busca
    res.status(500).render('plataforma/admin_usuarios', { 
        usuarios: [], // Lista vazia em caso de erro
        ...
        error: 'Ocorreu um erro ao buscar usuários.' // Mensagem para a view
    });
  }
};
...
export const aprovarSolicitacao = async (req, res) => {
  ...
  try {
    ...
    if (error.code === 'P2002') { // Email já existe
      ...
      console.error("Erro de email duplicado ao aprovar solicitação:", solicitacao?.email); // Adiciona verificação
