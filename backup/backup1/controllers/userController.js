import prisma from '../config/prisma.js'; // CORREÇÃO 1: Caminho e importação
import { z } from 'zod';
...
const formatRole = (tipo) => {
  // Seus ENUMs estão minúsculos no schema 
...
export const renderProfile = async (req, res) => {
  try {
    // CORREÇÃO 2: Usando req.user (do Passport)
    ...
    if (!user) {
      req.flash('error', 'Usuário não encontrado.'); // Agora temos flash
    ...
    // Usando os tipos de usuário minúsculos do schema 
    if (user.tipo === 'administrador' || user.tipo === 'professor') {
      ...
      const [cursosInscritos, aulasCanceladas, atividadesConcluidas] = await prisma.$transaction([
        prisma.inscricao.count({ where: { usuario_id: userId } }), // 
        prisma.progresso.count({ where: { inscricao: { usuario_id: userId }, aula_id: null } }), // Exemplo de atividades concluídas 
        prisma.progresso.count({ where: { inscricao: { usuario_id: userId }, atividade_id: null } }) // Exemplo de aulas concluídas  
      ]);
      // (Ajuste a lógica de stats de aluno conforme seu schema)
      stats = { label1: "Cursos", value1: cursosInscritos, label2: "Aulas", value2: 0, label3: "Atividades", value3: 0 };
    }
...
export const updateProfile = async (req, res) => {
  try {
    ...
    // CORREÇÃO 3: Atualiza a sessão do Passport
