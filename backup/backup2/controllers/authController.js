...
// ✅ NOTA: As rotas de login agora são tratadas pelo Passport em authRoutes.js
// Este controller só precisa de renderização e cadastro
...
export const processarCadastro = async (req, res) => {
  ...
  try {
    ...
    if (senha !== confirmar_senha) {
      return res.render('forms/cadastro', { 
    ...
    if (!validarSenha(senha)) {
      return res.render('forms/cadastro', { 
        error: 'A senha deve ter no mínimo 8 caracteres, incluindo maiúscula, número e símbolo (@$!%*?&).' 
      });
    }
    ...
    // 4. Validação adicional: instituição é obrigatória
    if (!instituicao) {
      return res.render('forms/cadastro', { 
        error: 'Por favor, selecione uma instituição.' 
      });
    }
    ...
    if (tipo === 'aluno') {
      ...
      // Solicitação de aprovação para professores
      ...
      req.flash('success', 'Solicitação enviada! Aguarde a aprovação do administrador.');
      res.redirect('/auth/login');
